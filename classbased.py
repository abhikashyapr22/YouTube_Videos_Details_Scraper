...
class YtScraper:
    ...
    def get_videos_url(self, ch_url: str, max_links: int, interaction_time: int = 1):
        """
        :param ch_url:
        :param max_links:
        ...
        print(f"found url {ch_url}")
        self.driver.get(ch_url)

        try:
            videos = self.driver.find_element(By.CSS_SELECTOR,
                                              'tp-yt-paper-tab.style-scope:nth-child(4) > div:nth-child(1)')
            videos.click()
        except:
            return ''
        ...
        while len(set(url_list)) < max_links:
            try:
                videos_list = self.driver.find_elements(By.CSS_SELECTOR,
    ...
    def get_video_details(self, ch_link):

        max_links = 1
        interaction_time = 1
        # driver.implicitly_wait(5)

        urls = self.get_videos_url(ch_link, max_links, interaction_time)
        ...
        for url in urls:
            ...
            try:
                views_el = self.driver.find_element(By.XPATH,
            ...
            try:
                likes = self.driver.find_element(By.XPATH, '//a/yt-formatted-string[@id="text"]').text  # working
            except:
                print("not able to extract likes")

            try:
                v_img = WebDriverWait(self.driver, 10).until(
            ...
            try:
                nc = self.driver.find_element(By.XPATH,
                                              '//*[@id="count"]/yt-formatted-string/span[1]').text  # working
            except:
            ...
            insert_data = VideoDetails(title=title if title else videotitle, url=url, views=views, likes=likes,
                                       comments=nc, thumbnail=v_img if v_img else videoimage)
            db.session.add(insert_data)
            db.session.commit()

        print(VideoDetails.query.all())

        return VideoDetails.query.all()
    ...
    def get_comments(self, url):
        ...
        while True:
            ...
            try:
                authors_list_el = self.driver.find_elements(By.CSS_SELECTOR,
            ...
            try:
                comments = self.driver.find_elements(By.CSS_SELECTOR, '#content.style-scope.ytd-expander')
        ...
        return zip(author_list, comment_list)
...
def get_video_details():
    if request.method == 'POST':
        ...
        video_info = YtScraper.get_video_details(ch_link)

        return render_template('result_test.html', vdata=video_info)
...
def get_comments():
    if request.method == "POST":
        ...
        data = YtScraper.get_comments(url)
